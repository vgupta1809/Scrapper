...
def load_autozone_data_from_csv(year)
  advance_auto_data = CSV.read('advance_autoparts_metadata.csv').select{|d| d if d[0] == year}
...
def target_model_match(retailer, make, m, model_word_count, models, oreillyauto_base_models)
  ...
  elsif (['ram'].include? make.downcase) && (['autozone', 'pepboys'].include? retailer.downcase)    
    target_models = models.map(&:downcase).select{ |c| oreillyauto_base_models.any?{ |k| k.gsub('ram ','')[0..1].start_with?(c.gsub('ram ','')[0..1]) && m.gsub('ram ','')[0..1].include?(k.gsub('ram ','')[0..1]) } if (m.gsub('ram ','')[0..1].include?(c.gsub('ram ','')[0..1])) }
  else
...
#binding.pry if make.downcase == 'ram' && m.downcase == '1500 Laramie Longhorn'.downcase
#  binding.pry if make.downcase == 'ram'
  #binding.pry if make.downcase == 'ram' && m.downcase == '1500 Laramie'.downcase
  (0..model_word_count-1).each do |count|
    tmp_m = m.gsub('-','').split[0..count].join(' ')
    if make.downcase == 'ram' 
      new_matched_models = target_models.select{ |c| c if c.gsub('-','').gsub('ram ','').split[0..count].join(' ').downcase == tmp_m.downcase }
      new_matched_models = target_models.select{ |c| c if c.gsub('-','').gsub('ram ','').start_with?(tmp_m) } if new_matched_models.empty?
      new_matched_models = target_models.select{ |c| c if tmp_m.start_with?(c.gsub('-','').gsub('ram ','').split[0..count].join(' ').downcase) } if new_matched_models.empty?
...
def pepboys_similar_model_match(e, make, oreillyauto_data, m, pepboys_models, oreillyauto_base_models, pepboys_data, rematch, model_word_count, ocylinder, osize, otype, ovin)
  ...
  target_models.sort.reverse.each do |tm|
    ...
    @pepboys_engine.sort_by(&:length).reverse.each do |ae|
      ...
      acylinder, asize, atype = engine_details(tm, ae, pepboys_data,'pepboys')
      unless atype.nil?
...
def autozone_similar_model_match(e, make, oreillyauto_data, m, autozone_models, oreillyauto_base_models, autozone_data, rematch, model_word_count, ocylinder, osize, otype, ovin)
  ...
  target_models = target_model_match('autozone', make, m ,model_word_count, autozone_models, oreillyauto_base_models)
    target_models.sort.reverse.each do |tm|
      ...
      #binding.pry if make == 'Chevrolet'.downcase && m == 'C3500 Pickup RWD CHEYENNE'.downcase && e =='V8 - 5.7L 350ci CNG MFI vin R - 2 valve OHV'
      ...
      @autozone_engine.sort_by(&:length).reverse.each do |ae|
        next if match_exist?(@master_engines[make], 'autozone', ae, rtm) unless rematch if make.downcase == 'ram'
        acylinder,asize ,atype, avin = engine_details(m, ae, autozone_data,'autozone')
        ##binding.pry if make == 'Chevrolet'.downcase && m == 'C3500 Pickup RWD CHEYENNE'.downcase && e =='V8 - 5.7L 350ci CNG MFI vin R - 2 valve OHV'
        unless atype.nil?
...
def advance_auto_similar_model_match(e, make, oreillyauto_data, m, advance_auto_models, oreillyauto_base_models, advance_auto_data, rematch, model_word_count, ocylinder, osize, otype, ovin)
  ...
  target_models.sort.reverse.each do |tm|
    ...
    @advance_auto_engine.sort_by(&:length).reverse.each do |ae|
      ...
      acylinder, asize, atype, avin = engine_details(m, ae, advance_auto_data,'advance_auto')
      if atype.nil? 
...
def oreillyauto_similar_model_match(e, make, m, oreillyauto_models, oreillyauto_base_models, oreillyauto_data, rematch, model_word_count, ocylinder, osize, otype, ovin)
  ...
  target_models.sort.reverse.each do |tm|
    ...
    @oreillyauto_engine.sort_by(&:length).reverse.each do |ae|
      ...
      acylinder, asize, atype, avin = engine_details(m, ae, oreillyauto_data,'oreillyauto')
...
def retailer_index_mapping
  {
    autozone: {cylinder: 9, asize: 8, aindex: 4, atype: 11, avin: 5},
    advance_auto: {cylinder: 9, asize: 6, aindex: 4, atype: 10, avin: 5},
    pepboys: {cylinder: 8, asize: 6, aindex: 4, atype: 9},
    oreillyauto: {cylinder: 13, asize: 10, aindex: 6, atype: 14, avin: 7}
  }
end

def engine_details(m,ae,data,retailer)
  ###binding.pry if  m == 'K1500 Pickup 4WD'.downcase && ae =='V8 - 6.5L 395ci DIESEL MFI Turbocharged vin F type L65 - 2 valve OHV' && retailer == 'autozone'
  aindex = data.find_index { |row| row if ae.strip == row[retailer_index_mapping[retailer.to_sym][:aindex]].strip && row[3].downcase == m.downcase }
  aindex = data.find_index { |row| row if ae.strip == row[retailer_index_mapping[retailer.to_sym][:aindex]].strip } if aindex.nil? 
  acylinder = data[aindex][retailer_index_mapping[retailer.to_sym][:cylinder]]
  asize = data[aindex][retailer_index_mapping[retailer.to_sym][:asize]].delete("L ") rescue nil
  atype = data[aindex][retailer_index_mapping[retailer.to_sym][:atype]]
  avin = data[aindex][retailer_index_mapping[retailer.to_sym][:avin]].delete('-') rescue nil unless retailer == 'pepboys'
...
def pepboys_engine_mapper(d,e,advance_auto_data, autozone_data, oreillyauto_data, pepboys_data, make, m, ocylinder, osize, otype, ovin)
  if make.downcase == 'subaru'
    @pepboys_engine.sort_by(&:length).reverse.each do |ae|
      ...
      acylinder, asize, atype = engine_details(@pepboys_model, ae, pepboys_data,'pepboys')
...
def oreillyauto_engine_mapper(d,e,advance_auto_data, autozone_data, oreillyauto_data, pepboys_data, make, m, ocylinder, osize, otype, ovin)
  ...
  @oreillyauto_engine.sort_by(&:length).reverse.each do |ae|
    ...
    acylinder, asize, atype = engine_details(m, ae, oreillyauto_data,'oreillyauto')
...
def autozone_engine_mapper(d,e,advance_auto_data, autozone_data, oreillyauto_data, pepboys_data, make, m, ocylinder, osize, otype, ovin)
  #&& m == 'RAM 1500 Laramie Longhorn 2WD'.downcase && e =='6 Cylinders M 3.0L Turbo Dsl DOHC'
  if make.downcase == 'subaru'
   @autozone_engine = autozone_data.select{|d| d if d[1].downcase == make.downcase && d[3] == @autozone_model}.map{ |d| d[4]}
   @autozone_engine.sort_by(&:length).reverse.each do |ae|
...
if d[:autozone].nil?
  @autozone_engine.sort_by(&:length).reverse.each do |ae|
    ...
    break unless d[:autozone].nil?
    ##binding.pry if make == 'Gmc'.downcase && m == 'C2500 Pickup RWD Sierra SLE'.downcase && e =='V8 - 5.7L 350ci CNG MFI vin R - 2 valve OHV'
    acylinder,asize ,atype, avin = engine_details(m, ae, autozone_data,'autozone')
    unless avin.nil? || ovin.nil?
...
if d[:autozone].nil?
    ...
    @autozone_engine.sort_by(&:length).reverse.each do |ae|
      ...
      unless avin.nil? || ovin.nil?
        if atype.nil?
          ...
          if (make =='gmc' || make =='chevrolet') && otype == 'Natural Gas'
            d[:autozone] = ae.strip, @autozone_model if asize == osize && acylinder == ocylinder && atype == otype 
          else
  ...
  #binding.pry if make == 'Chevrolet'.downcase && m == 'K2500 Pickup 4WD'.downcase && e =='V8 - 6.5L 395ci DIESEL MFI Turbocharged vin S type L56 - 2 valve OHV'
  make_eleminate = ['chevrolet','gmc','ford']
  check_for_ford_model = /^[a-z]{1,4}\d{4}$/i
  #binding.pry if make == 'ford' && m.downcase =='LTLA9000'.downcase
  if make_eleminate.include?(make.downcase)
    return d[:autozone] unless make.downcase=="ford" && check_for_ford_model.match(m.downcase)
  end
  if d[:autozone].nil?
    @autozone_engine.sort_by(&:length).reverse.each do |ae|
      next if match_exist?(@master_engines[make], 'autozone', ae, @autozone_model)
      break unless d[:autozone].nil?
      ###binding.pry if make == 'Chevrolet'.downcase && m == 'K1500 Pickup 4WD'.downcase
      acylinder,asize ,atype, avin = engine_details(m, ae, autozone_data,'autozone')
      unless atype.nil?
        d[:autozone] = ae.strip, @autozone_model if asize == osize && acylinder == ocylinder && atype == otype
      else
...
def lone_entries_to_master(year, models, retailer, data, make, match, wd)
  ...
  models.each do |m|
    ...
    engine = pepboys_data.select{|d| d if d[3] == m}.map{ |d| d[4]} if retailer == 'pepboys'
    engine = advance_auto_data.select{|d| d if d[3] == m}.map{ |d| d[4]} if retailer == 'advance_auto'
    engine = autozone_data.select{|d| d if d[3] == m}.map{ |d| d[4]} if retailer == 'autozone'
    engine.sort_by(&:length).reverse.each do |e|
      ...
      #binding.pry if retailer == 'autozone' && make == 'Ram'.downcase
      model_data = {}
      e = e.strip
      next if match_exist?(@master_engines[make], retailer, e, m) unless retailer == 'autozone'
      next if match_exist?(@master_engines[make], retailer, e, m) if match if retailer == 'autozone'
      next unless (m.downcase.include?('awd') || m.downcase.include?('4wd') || m.downcase.include?('2wd')) if wd if retailer == 'autozone'
      ocylinder,osize ,otype = engine_details(m, e, pepboys_data,'pepboys') if retailer == 'pepboys'
      ocylinder,osize ,otype = engine_details(m, e, advance_auto_data,'advance_auto') if retailer == 'advance_auto'
      #binding.pry if retailer == 'autozone' && make == 'Ram'.downcase
      ovin = nil
      ocylinder,osize ,otype, ovin = engine_details(m, e, autozone_data,'autozone') if retailer == 'autozone'
      ...
      d = {}
      d[:advance_auto] = e.strip, m if retailer == 'advance_auto'
      d[:advance_auto] = advance_auto_engine_mapper(d, e, advance_auto_data, autozone_data, oreillyauto_data, pepboys_data, make, m, ocylinder, osize, otype, ovin) unless retailer == 'advance_auto'
      ...
      d[:pepboys] = e.strip, m if retailer == 'pepboys'
      d[:pepboys] = pepboys_engine_mapper(d, e, advance_auto_data, autozone_data, oreillyauto_data, pepboys_data, make, m, ocylinder, osize, otype, ovin) unless retailer == 'pepboys'
      
      d[:autozone] = e.strip, m if retailer == 'autozone'
      d[:autozone] = autozone_engine_mapper(d, e, advance_auto_data, autozone_data, oreillyauto_data, pepboys_data, make, m, ocylinder, osize, otype, ovin) unless retailer == 'autozone'
...
def retailer(csv_data, year)
  ...
  csv_data.each do |t|
    ...
    if master_engine.strip == t[4]
      #binding.pry if t[0].downcase== 'ram' && t[2].downcase == '6 Cylinders M 3.0L Turbo Dsl DOHC'.downcase
      acylinder, asize, atype, avin = engine_details(t[1], master_engine, autozone_data,'autozone')
      retailer = 'autozone'  
    end

    if master_engine.strip == t[6]
      acylinder, asize, atype, avin = engine_details(t[1], master_engine, advance_auto_data,'advance_auto')
      retailer = 'advance auto'
    end

    if master_engine.strip == t[8]
      acylinder, asize, atype = engine_details(t[1], master_engine, pepboys_data,'pepboys')
      avin = nil
      retailer = 'pepboys'
    end

    if master_engine.strip == t[10]
      acylinder, asize, atype, avin = engine_details(t[1], master_engine, oreillyauto_data,'oreillyauto')
    end
  ...
  makes.map(&:downcase).uniq.each do |make|  
    ...
    oreillyauto_models.map(&:downcase).uniq.each do |m|
      ...
      @oreillyauto_engine.sort_by(&:length).reverse.each do |e|
        ...
        load_autopats_engines(make, advance_auto_data, autozone_data, oreillyauto_data, pepboys_data)
        ocylinder, osize, otype, ovin = engine_details(m, e, oreillyauto_data,'oreillyauto')
    ...
    lone_entries_to_master(year, autozone_models, 'autozone', autozone_data, make, true, false)
    lone_entries_to_master(year, advance_auto_models, 'advance_auto', advance_auto_data, make,nil,nil)
    lone_entries_to_master(year, pepboys_models, 'pepboys', pepboys_data, make,nil,nil)
  end
