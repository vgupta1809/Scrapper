...
def load_autozone_data_from_csv(year)
  advance_auto_data = CSV.read('advance_auto.csv').select{|d| d if d[0] == year}
  autozone_data = CSV.read('autozone.csv').select{|d| d if d[0] == year}
  oreillyauto_data = CSV.read('oreillyauto.csv').select{|d| d if d[0] == year}
  pepboys_data = CSV.read('pepboys.csv').select{|d| d if d[0] == year}
...
def write_file(csv_data,year)
  File.open("#{year}_master_models_data_final.csv", "w") do |f|
